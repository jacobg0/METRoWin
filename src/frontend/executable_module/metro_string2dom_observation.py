...
class Metro_string2dom_observation(Metro_string2dom):
    ...
    def __string2dom(self):
        ...
        if self.infdata_exist('OBSERVATION'):
            pObservation = self.get_infdata_reference('OBSERVATION')
            sObservation = pObservation.get_input_information()
            try:
                domObservation = self._convert_string2dom(sObservation)
            except metro_error.Metro_xml_error, inst:
                sMessage = _("Fatal Error when converting observation ") +\
                           _("string to DOM. The error is:\n%s") % (str(inst))
                metro_logger.print_message(metro_logger.LOGGER_MSG_STOP,
                                       sMessage)
            else:
                pObservation.set_input_information(domObservation)
        else:
            sMessage = _("Fatal Error, no observation string to convert.")
            metro_logger.print_message(metro_logger.LOGGER_MSG_STOP,
                                       sMessage)
